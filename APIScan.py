...
#custom scripts
from db_operations import write_list_to_db, set_status_back_to_ready, get_date, initialize_db, get_db_config
...
def main():
        ...
        process_id = initialize_db() #Unique ProcessID will be given by the database AutoIncrement column       
        met_dept_api_url_list = create_url_list() #get the url_list (refer to html_operations.py for the hardcoded values)
               
        while True:
            current_url_date = get_date(process_id) #looks for a data assigned to this process id, repeats every 2 seconds till there is an assignment
            data_list = []
